...
2. **Codex CLI：軟體工程主管**
   ...
   - 制定 worker 預檢規則、判讀預檢結果、裁決是否准派或改派。
...
4. **Dispatcher／Worker Wrapper：可靠性邊界**
   ...
   - Wrapper 負責執行即時 quota / auth / cooldown 預檢；Dispatcher 依預檢結果執行派工、fallback 與重試。
...
- **worker 的即時可用性預檢在 Wrapper；預檢規則與准派 / 改派裁決在 `ctrl-codex`；實際派工流程在 Hermes / Dispatcher。**
...
`ctrl-codex` 不直接代替 Worker 做即時 quota 探測；它只制定預檢規則並讀取 Wrapper 回傳狀態。
...
- `ctrl-codex` 定義何時必須 probe、如何分類 80/81/82，以及何時允許改派。
- Wrapper 只負責執行 probe、保存原始輸出、回填 `retry_after` 與標準化結果。
- Hermes / Dispatcher 根據預檢結果決定是否派工、進入 cooldown、或切換 fallback agent。
...
- `ctrl-codex` 定義 Claude Code 預檢欄位、失敗分類與准派規則。
- Wrapper 負責執行低成本 probe 與錯誤分類，不自行決定永久改派。
- Hermes / Dispatcher 依預檢結果執行 fallback、排隊與重試。
...
3. 讀取或觸發 Wrapper 預檢結果。
4. 只對預檢通過的 Worker 建立獨立 branch/worktree。
5. 將 execution manifest 透過 stdin 或受控檔案傳送。
6. 使用 `timeout`/cgroup 限制執行時間。
7. 收集 stdout/stderr。
8. 驗證結果 JSON。
9. 檢查 changed paths。
10. 執行 acceptance commands。
11. 保存 commit、patch、結果與日誌。

角色補充：

1. `ctrl-codex`：定義預檢必填欄位、分類標準與准派 / 改派規則。
2. Wrapper：執行即時預檢並輸出標準化 `preflight` / `result` JSON。
3. Dispatcher：消費預檢結果並執行派工、fallback、cooldown、重試。
...
5. `ctrl-codex` 定義該 task 的預檢規則與准派條件。
6. Dispatcher / Wrapper 對目標 Worker 執行預檢。
7. 預檢通過後才正式派工；未通過則依規則改派或排隊。
8. Worker 執行。
9. Codex 驗收。
10. Hermes 通知。
11. 人工決定 PR、合併與部署。
...
3. **即時額度偵測在 Wrapper，預檢規則與准派裁決在 `ctrl-codex`，調度與 fallback 執行在 Hermes / Dispatcher。**
