...
- **Codex CLI 受 OpenAI API 速率限制（RPM、TPM、RPD）約束，需由 Wrapper 偵測並退避。**
- **Claude Code 受 Anthropic API 速率限制（requests/tokens per minute/day）及 Max plan 用量限制約束，需由 Wrapper 偵測並退避。**
...
| B | `ctrl-codex` | 工程規劃與驗收 | Codex CLI、Git、GitHub CLI、專案工具鏈、Tailscale |
| C | `worker-opencode` | OpenCode 任務 | OpenCode、Git、專案工具鏈、SSH server、Tailscale |
| D | `worker-claude` | Claude Code 任務 | Claude Code、Git、專案工具鏈、SSH server、Tailscale |
...
| 78 | 修改範圍越界 |
| 79 | 驗收失敗 |
| 80 | Codex CLI API 速率限制（RPM/TPM 短期） |
| 81 | Codex CLI API 每日用量限制（RPD） |
| 82 | Codex CLI API 認證失效或無效金鑰 |
| 83 | Claude Code API 速率限制（短期 429） |
| 84 | Claude Code API 每日／月用量上限 |
| 85 | Claude Code 認證失效或 OAuth token 過期 |
...
Codex CLI 使用 OpenAI API，受 **RPM（每分鐘請求數）**、**TPM（每分鐘 Token 數）** 與 **RPD（每日請求數）** 限制。

### 限制類型辨識

| HTTP 狀態 | 錯誤碼／訊息特徵 | 分類 | Exit Code |
|---|---|---|---|
| 429 | `rate_limit_exceeded`、`requests per minute` | 短期 RPM/TPM | 80 |
| 429 | `requests per day`、`daily limit` | RPD 每日上限 | 81 |
| 401 / 403 | `invalid_api_key`、`Unauthorized` | 認證失效 | 82 |
...
執行 codex exec / codex run
  ├─ exit 0 → 正常完成
  ├─ stderr 含 429 + RPM/TPM 關鍵字
  │     → exit 80，寫入 retry_after（Retry-After header 或預設 60s）
  ├─ stderr 含 429 + RPD/daily 關鍵字
  │     → exit 81，寫入 retry_after（隔日 00:00 UTC 或預設 24h）
  ├─ stderr 含 401/403 / invalid_api_key
  │     → exit 82，停用 Worker，通知人工
  └─ 其他非零 → exit 70
```

### 重試策略

- **exit 80（RPM/TPM）**：指數退避，最小 60 秒，最大 10 分鐘；同一 API key 的並行任務應減少。
- **exit 81（RPD）**：寫入隔日重試，停止向該 API key 派工直到 reset；有 fallback agent 時立即切換。
- **exit 82（認證）**：停用 Worker、發出人工通知，不自動重試。
- **API key 共享**：同一 API key 使用於多台 Worker 時，quota 狀態應共用（同 Antigravity 帳號邏輯）。

### 探測方式

Codex CLI 沒有專屬 `/quota` 指令。Wrapper 可在正式任務前發送最小化 probe（如單行 `echo` 任務），以低成本確認 API key 仍有效且未觸發速率限制。

---

## 12c. Claude Code 速率與用量限制偵測

Claude Code 使用 Anthropic API（Pro/Max plan 或 API key），受 **RPM**、**tokens per minute**、**每日請求上限** 及 **Max plan 月用量** 約束。
...
| 401 | `authentication_error`、`invalid_x_api_key`、OAuth token expired | 認證失效 | 85 |
...
執行 claude -p ... --output-format json
  ...
  ├─ stderr/JSON error 含 authentication_error / invalid_x_api_key / OAuth expired
...
- **exit 85（認證）**：停用 Worker、發出人工通知；OAuth token 過期時引導人工重新登入，不自動嘗試刷新 token。
- **帳號隔離**：Max plan 用量綁定 Anthropic 帳號，同帳號多台 Worker 共用同一 quota pool，狀態應集中記錄。
...
- 任務完成且 Codex 驗收通過。
- 任務失敗且無 fallback。
- 登入失效（包含 Codex CLI API key 失效、Claude Code OAuth 過期）。
- weekly quota（Antigravity）。
- Codex CLI 觸發 RPD 每日上限（exit 81）。
...
- 兩個 task 宣告修改同一檔案。
- Prompt 要求讀取私鑰。
- Codex CLI 觸發 RPM 限制，確認指數退避正確啟動。
- Codex CLI API key 失效，確認停用 Worker 且通知人工。
