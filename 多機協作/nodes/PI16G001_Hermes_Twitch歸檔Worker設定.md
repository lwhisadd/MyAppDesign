...
> 文件用途：保存 PI16G001 上由 Hermes 管理的 Twitch 定時下載任務變數。  
> 適用範圍：`多機協作/` context。  
...
所有 Hermes / Codex / Worker Agent 在規劃、建立、派送或執行 Twitch 歸檔任務前，必須先讀取本文件。
...
2. 解析 `HER​MES_TWITCH_ARCHIVE_VARIABLES` 區塊中的變數。
...
```yaml
HER​MES_TWITCH_ARCHIVE_VARIABLES:
  task_profile: "twitch_daily_vod_archive"
  project: "twitch-archive"
  context_root: "多機協作"

  hermes:
    controller_id: "ctrl-hermes"
    control_root: "/srv/hermes-control"
    task_root: "/srv/hermes-control/tasks"
    state_db: "/srv/hermes-control/state/coordination.db"
  ...
  worker:
    ...
    memory: "16GB"
    worker_root: "/srv/agent"
    runner_path: "/srv/agent/bin/run-twitch-archive"
...
```text
依照 PI16G001_Hermes_Twitch歸檔Worker設定.md 中的 HER​MES_TWITCH_ARCHIVE_VARIABLES，下載 TWITCH_CHANNEL_SOURCES 內啟用來源的新 Twitch VOD，避免重複下載，並產出任務結果摘要。
```
...
- 任務狀態與派工真相應寫入 Hermes DB，不依賴對話記憶。
